...
def calculate_score_for_test_data_chain():
    ...
    y3 = mean_recall('3')
    y4 = mean_recall('4')
    y4_u = mean_recall('4_units')
    y5 = mean_recall('5')
    y6 = mean_recall('6')
    y7 = mean_recall('7')
    
    output_dict['hit_ratio'] = {'y3':y3, 'y4_u':y4_u, 'y4':y4, 'y5':y5, 'y6':y6, 'y7':y7}
    ...
    data_preproc = pd.DataFrame({
        ...
        'Proxy Observation(chain len = 2)': y3,
        'Interpretation Var(chain len = 3)': y4,
        'Proxy Units(chain len = 3)': y4_u,
        'Interpretation Var Detail(chain len = 4)': y5,
        'Inferred Var(chain len = 5)': y6,
        'Inferred Var Units(chain len = 6)': y7})
    ...
    output_dict['mrr'] = {'y3':y3, 'y4_u':y4_u, 'y4':y4, 'y5':y5, 'y6':y6, 'y7':y7}
    ...
    with open('lstm_author_metrics_data', 'w') as output_file:
