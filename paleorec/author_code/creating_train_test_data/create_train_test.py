...
def read_latest_data_for_training():
    ...
    data_file_path = fileutils.get_latest_file_with_path(data_file_dir, 'merged_common_lipdverse_inferred_auth_*.csv')
...
def write_autocomplete_data_file():
    ...
    if _platform == "win32":
        autocomplete_file_path = '..\\data\\autocomplete\\autocomplete_file_auth_'+timestr+'.json'
    else:
        autocomplete_file_path = '../data/autocomplete/autocomplete_file_auth_'+timestr+'.json'
...
def discard_less_frequent_values_from_data():
    ...
    final_df = common_lipdverse_df.filter(['authorName','filename','archiveType','proxyObservationType', 'units', 'interpretation/variable', 'interpretation/variableDetail', 'inferredVariable', 'inferredVarUnits'], axis=1)
...
def calculate_counter_info(final_df):
    ...
    counter_author = collections.Counter(final_df['authorName'])
    ...
    final_ground_truth_dict.update({'auth_names':list(counter_author.keys()),'archive_types': list(counter_archive.keys()),'proxy_obs_types': list(counter_proxy.keys()),'units': list(counter_units.keys()),'int_var': list(counter_int_var.keys()),'int_var_det': list(counter_int_det.keys()), 'inf_var': list(counter_inf_var.keys()), 'inf_var_units': list(counter_inf_var_units.keys())}) 
...
def downsample_archives_create_final_train_test_data():
    ...
    df_rest = df_rest.filter(['authorName', 'archiveType','proxyObservationType', 'units', 'interpretation/variable', 'interpretation/variableDetail', 'inferredVariable', 'inferredVarUnits'], axis=1)
    ...
    if _platform == "win32":
        lipd_downsampled_path = '..\data\csv\lipdverse_downsampled_auth_'+timestr+'.csv'
        lipd_test_path = '..\data\csv\lipdverse_test_auth_'+timestr+'.csv'
        ground_truth_path = '..\data\ground_truth_info\ground_truth_label_auth_'+timestr+'.json'
    else:
        lipd_downsampled_path = '../data/csv/lipdverse_downsampled_auth_'+timestr+'.csv'
        lipd_test_path = '../data/csv/lipdverse_test_auth_'+timestr+'.csv'
        ground_truth_path = '../data/ground_truth_info/ground_truth_label_auth_'+timestr+'.json'
    ...
    print('\n Creating file {} at location {}'.format('lipdverse_downsampled_auth_'+timestr+'.csv', lipd_downsampled_path[:lipd_downsampled_path.rindex('lipdverse_down')]))
    ...
    print('\n Creating file {} at location {}'.format('lipdverse_test_auth_'+timestr+'.csv', lipd_test_path[:lipd_test_path.rindex('lipdverse_test')]))
    final_df_test.to_csv(lipd_test_path, sep = ',', encoding = 'utf-8',index = False)

    print('\n Creating file {} at location {}'.format('ground_truth_label_auth_'+timestr+'.json', ground_truth_path[:ground_truth_path.rindex('ground_truth_label')]))
