...
class LSTMpredict:
    ...
    def __init__(self, model_file_path, ground_truth_file_path, topk):
        ...
        GROUND_TRUTH_FILE_PATH = get_latest_file_with_path(ground_truth_file_path, 'ground_truth_label_auth_*.json')
        ...
        self.names_set = {0 : set(self.ground_truth['auth_names']), 1 : set(self.ground_truth['archive_types']), 2: set(self.ground_truth['proxy_obs_types']), 3: set(self.ground_truth['units']), 4: set(self.ground_truth['int_var']), 5: set(self.ground_truth['int_var_det']), 6: set(self.ground_truth['inf_var']), 7: set(self.ground_truth['inf_var_units'])}
        for i in range(7):
    ...
    def predictForSentence(self, sentence, isInferred = False):
        ...
        if isInferred and len(input_sent_list) <= 3:
            
            inferredVar = None
            if len(input_sent_list) == 3:
                inferredVar = input_sent_list[2]
                if inferredVar not in self.names_set[6]:
                    return {'0': []}
                del input_sent_list[2]
            while(len(input_sent_list) < 5):
            ...
            names_set_ind = len(input_sent_list) + 1 if len(input_sent_list) >= 3 else len(input_sent_list)
