...
def calculate_unique_chains(dataframe_obj):
    ...
    chain1 = dataframe_obj[['authorName','archiveType', 'proxyObservationType','units']]
    
    chain2 = dataframe_obj.filter(['authorName','archiveType', 'proxyObservationType', 'interpretation/variable', 'interpretation/variableDetail', 
...
def get_data_from_df(lipd_data_df, batch_size, seq_size):
    ...
    if for_units:
        lipd_data = lipd_data_df.filter(['authorName','archiveType', 'proxyObservationType', 'units'])
    else:
        lipd_data = lipd_data_df.filter(['authorName','archiveType', 'proxyObservationType', 'interpretation/variable', 'interpretation/variableDetail', 'inferredVariable', 'inferredVarUnits'])
...
def print_save_loss_curve(train_loss_list, chain):
    ...
    final_path = loss_curve_path + chain + 'training_loss_e_' + str(epochs) + '_l_'+ str(learning_rate) + '_auth_' + timestr + '.png'
...
def main():
    ...
    if not for_units:
        ...
        # Train for author -> archive -> proxyObservationType -> interpretation/variable -> interpretation/variableDetail -> inferredVariable -> inferredVarUnits
        ...
        # Train for author -> archive -> proxyObservationType -> units
...
train_path = fileutils.get_latest_file_with_path(data_file_dir, 'lipdverse_downsampled_auth_*.csv')
